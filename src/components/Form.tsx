...
const Form: React.FC<props> = ({ user, setUser, handleSubmit }) => {
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const userValue = { ...user, [e.target.name]: e.target.value };
    setUser(userValue);
  ...
  return (
    <div className="border-2 ml-10 text-center rounded bg-blue-100 h-80">
      ...
      <div className=" text-left grid  gap-2 mt-4 ml-8 mr-8 py-2 px-4">
        ...
        {user.id ? (
          <button
            className=" bg-sky-600 hover:bg-sky-700 py-2 px-4 w-32 rounded mt-2 ml-2"
          ...
          <button
            className="bg-green-400  active:hover-bg-yellow py-2 px-4 w-32 rounded mt-2 ml-2"
            onClick={handleSubmit}
          >
            SUBMIT
          </button>
